...
class reportbiayaproject implements FromCollection, WithEvents, ShouldAutoSize, WithStyles, WithHeadings
{
    use Exportable;
    ...
    protected $_DataProjectPrefixSrv = null;
    protected $_DataProject = null;
    

    public function __construct($data_result, $ProjectAlias, $ftDateStart, $ftDateEnd, $DataProjectPrefixSrv, $DataProject)
    {
        $this->_data_result =  $data_result;
        $this->_DataProjectPrefixSrv =  $DataProjectPrefixSrv;
    ...
    public function collection()
    {
        ...
        return $data_result->map(function ($cdr_data) {
            ...
            if($cdr_data != 'a'){
                ...
                if (isset($this->_DataProjectPrefixSrv[$cdr_data['idxCorePrefix']])) {
                    $PrefixNumber = $this->_DataProjectPrefixSrv[$cdr_data['idxCorePrefix']];
                }

                return [
                    'idxCorePrefix'      => $PrefixNumber,
                    'idxCoreProject'          => $aliasProject,
                    // 'Sum_jmlWaktuReal'    => $cdr_data['Sum_jmlWaktuReal'],
                    // 'Sum_jmlWaktuTagih'     => $cdr_data['Sum_jmlWaktuTagih'],
                    // 'Sum_jmlWaktuReal'    => number_format($cdr_data['Sum_jmlWaktuReal'], 0, ',', '.'),
                    'Sum_jmlWaktuReal'    => number_format($cdr_data['Sum_jmlWaktuReal'], 0, ',', ','),
                    'Sum_jmlWaktuTagih'    => number_format($cdr_data['Sum_jmlWaktuTagih'], 0, ',', ','),
                    'Sum_biayaTagih'  => number_format($cdr_data['Sum_biayaTagih'], 0, ',', ',')
                ];
