...
class ReportCdrDetil implements FromCollection, WithEvents, ShouldAutoSize, WithStyles, WithHeadings
{
    ...
    public function __construct($data_result, $ProjectAlias, $ftDateStart, $ftDateEnd, $priceGroup)
    {
        // echo $priceGroup;exit;
    ...
    public function collection()
    {
        ...
        return $data_result->map(function ($cdr_data) {
            ...
            if($cdr_data != 'a'){
                ...
                if (!empty($this->_priceGroup)) {
                    $tarifPerMenit = $this->_priceGroup;
                }else{
                    $tarifPerMenit = $cdr_data['custprice'];
                }
                $getMenit_custTimeCDR = $cdr_data['Duration']/60 ;
                $TotalPrice =  $getMenit_custTimeCDR * $tarifPerMenit;
                // $TotalPrice =  ceil($getMenit_custTimeCDR * $tarifPerMenit);

                $this->_All_WaktuReal += (int)$cdr_data['WaktuReal'];
                $this->_All_Duration += (int)$cdr_data['Duration'];
                $this->_All_TotalPrice += (int)$TotalPrice;
                return [
                    ...
                    'WaktuReal'    => $cdr_data['WaktuReal'],
                    'Duration'     => $cdr_data['Duration'],
                    'DurationMin'  => $tarifPerMenit,
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function(AfterSheet $event) {
                ...
                $WaktuRealExc = $this->_All_WaktuReal;
                $DurationExc = $this->_All_Duration;
                $TotalPriceExc = $this->_All_TotalPrice;
                // echo $TotalPriceExc;exit;
