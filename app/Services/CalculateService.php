...
class CalculateService
{
    ...
    public function readEDirect($valuesArray, $setting_parser) {
        ...
        foreach ($columns as $column) {
            ...
            switch ($column) {
                case 'elapsedTime':
                    $cdrdata[$column] = Null;
                    $elapsedTime = $this->getColumnData($valuesArray, $setting_parser, $column);
                    if (is_numeric($elapsedTime)) {
                        $cdrdata[$column] = $elapsedTime;
                    }
                break;
                case 'datetime':
                    $cdrdata[$column] = Null;
                    $datetime = $this->getColumnData($valuesArray, $setting_parser, $column);
                    if ($this->isDateTimeValid($datetime)) {
                        $cdrdata[$column] = $datetime;
                    }
                break;
                case 'sourceNo':
    ...
    public function isDateTimeValid($datetime)
    {
        // Pengecekan menggunakan fungsi strtotime() untuk memastikan $datetime adalah tanggal
        ...
        return false; // Jika strtotime() gagal, bukan tanggal
    }
}