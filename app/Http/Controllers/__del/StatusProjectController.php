...
class StatusProjectController extends Controller
{
    ...
    private $_page_title = 'Status Project';
    private $_url_data = 'statusproject';
    private $_id_role = '';
    ...
    public function index()
    {
        ...
        {
            return view('master.statusproject.index', [
    ...
    public function create()
    {
        ...
        if ( !empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest' )
        {
            return view('master.statusproject.create', [
                'page_title' => $this->_page_title
            ]);
        }else{
            // return abort(404);
    ...
    public function edit(string $id)
    {
        if ( !empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest' )
        {
            $statusproject = statusproject::findOrFail($id);
            return view('master.statusproject.edit', [
                'page_title' => $this->_page_title,
                'data' => $statusproject
            ]);
        }else{
    ...
    public function update(Request $request, string $id)
    {
        ...
        if($request->ajax())
        {
            $updateData = statusproject::findOrFail($id);
            $validation = Validator::make($request->all(), [
                "name" => ['required', 'min:3', 'max:255'],
    ...
    public function destroy($work_location)
    {
        if ( !empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest' )
        {
            $del = statusproject::findOrFail($work_location);
            if(!empty($del)){
                if ($del->delete()){
    ...
    public function load(Request $request)
    {
        if($request->ajax())
        {
            ...
            foreach ($data_result as $value) {
                $id = $value->id;
                $linkdelete =  url('statusproject/'.$id);
                ...
                $action .= "<a href='javascript:void(0);' 
                    ...
                    data-module='statusproject' 
                    data-url='statusproject/{$id}/edit' 
