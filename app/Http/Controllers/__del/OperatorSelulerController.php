...
class OperatorSelulerController extends Controller
{

    private $_access_menu;
    private $_page_title = 'Operator Seluler';
    private $_url_data = 'operatorseluler';
    private $_id_role = '';
    ...
    public function index()
    {
        ...
        {
            return view('master.operatorseluler.index', [
    ...
    public function create()
    {
        ...
        if ( !empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest' )
        {
            return view('master.operatorseluler.create', [
                'page_title' => $this->_page_title
            ]);
        }else{
            // return abort(404);
    ...
    public function edit(string $id)
    {
        if ( !empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest' )
        {
            $operatorseluler = operatorseluler::findOrFail($id);
            return view('master.operatorseluler.edit', [
                'page_title' => $this->_page_title,
                'data' => $operatorseluler
            ]);
        }else{
            return redirect('./#dashboard');
        }
    ...
    public function update(Request $request, string $id)
    {
        ...
        if($request->ajax())
        {
            $updateData = operatorseluler::findOrFail($id);
            $validation = Validator::make($request->all(), [
                "name" => ['required', 'min:3', 'max:255'],
    ...
    public function destroy($work_location)
    {
        if ( !empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest' )
        {
            $del = operatorseluler::findOrFail($work_location);
            if(!empty($del)){
                if ($del->delete()){
    ...
    public function load(Request $request)
    {
        if($request->ajax())
        {
            ...
            foreach ($data_result as $value) {
                $id = $value->id;
                $linkdelete =  url('operatorseluler/'.$id);
                ...
                $action .= "<a href='javascript:void(0);' 
                    ...
                    data-module='operatorseluler' 
                    data-url='operatorseluler/{$id}/edit' 
