...
class WorkLocationController extends Controller
{
    ...
    private $_page_title = 'Work Location';
    private $_url_data = 'worklocation';
    ...
    public function index()
    {
        ...
        {
            return view('content.worklocation.index', [
    ...
    public function create()
    {
        ...
        if ( !empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest' )
        {
            return view('content.worklocation.create', [
                'page_title' => $this->_page_title
            ]);
        }else{
            // return abort(404);
    ...
    public function edit(string $id)
    {
        if ( !empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest' )
        {
            ...
            return view('content.worklocation.edit', [
                'page_title' => $this->_page_title,
                'data' => $worklocation
            ]);
        }else{
    ...
    public function update(Request $request, string $id)
    {
        ...
        if($request->ajax())
        {
            $updateData = Worklocation::findOrFail($id);
            $validation = Validator::make($request->all(), [
                "name" => ['required', 'min:3', 'max:255'],
                "address" => ['required', 'min:3', 'max:255'],
                "phone" => ['required', 'min:3', 'max:255']
    ...
    public function destroy($work_location)
    {
        if ( !empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest' )
        {
            $del = Worklocation::findOrFail($work_location);
            if(!empty($del)){
                if ($del->delete()){
    ...
    public function load(Request $request)
    {
        if($request->ajax())
        {
            ...
            $data_result = Worklocation::where($cond)->offset($offset)->limit($limit)->get();
            $data_count = Worklocation::where($cond)->count();
            ...
            foreach ($data_result as $value) {
                ...
                $action .= "<a href='javascript:void(0);' 
                    ...
                    data-module='worklocation' 
                    data-url='worklocation/{$id}/edit' 
